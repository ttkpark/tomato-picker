...
/* ===================== 차체 / 트레이 ===================== */
chassis_w = 151.9; // [실측] 차체 상판 가로 (X) — 2026-09-26 버니어 캘리퍼스 실측
chassis_d = 195.5; // [실측] 차체 상판 세로 (Y) — 2026-09-26 줄자 실측(앞 끝→뒤 끝)
tray_h    = 70;    // 젯슨+방열팬 높이 여유 (젯슨 보드 21 + 팬·방열판 ~30 + 배선)

// 차체 상판 체결 패턴 — [STL] TOPLeft·TOPRight 공용: M3, 50 × 30
mount_dx = 50;  mount_dy = 30;
mount_centers = [[chassis_w/2, 14.75], [chassis_w/2, 180.75]]; // [실측] 두 벌 중심 (중심간격 166.0mm, 대향나사 136.0mm 실측)
...
rot_center = [chassis_w/2, 98];   // [실측] 회전 중심 — 차체 중앙 근처 (195.5/2 = 97.75mm)
