...
| 부품 | 들어가는 것 |
|---|---|
| `tray` | 151.9 × 225 × 70. **앞=아두이노**(+바닥 카메라 선반), 가운데 왼쪽=벅부스트 2개, 오른쪽 벽=전압·전류계 창, **뒤=젯슨**(단자는 뒷벽). 바닥이 차체 상판의 **M3 50×30 패턴 두 벌(중심 간격 166)**에 그대로 박힌다. 전면 "ForNerds" 양각 |
...
- **배터리는 트레이에 없다** — 4S3P 팩이 들어갈 자리가 안 남는다. 지금처럼 차체 아래(BOTTOM)에 둔다(크기 실측 후 재검토).
