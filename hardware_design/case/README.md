...
| `platform` | SO-101 베이스 구멍, Orbbec 1/4" 나사 자리, 혼까지 내려가는 연결 기둥(7.7, 혼 나사 M3×16), 베어링 상부 링, 스토퍼 핀 |
...
| 조건 | 값 | 이유 |
|---|---|---|
| 종류 | **강철 볼 · 원형 · "헤비 듀티"**, 두 링에 각각 나사 구멍 | 케이크 장식용 얇은 알루미늄 링은 볼이 플라스틱이고 기울기 유격이 커서 팔 끝이 흔들린다 |
| 바깥지름 | **≤ 145** (5.5인치 ≈ 140 권장) | 뚜껑 폭 151.9 |
| 안지름 | **≥ 90** | 안쪽에 혼(Ø20)·배선 슬롯(r 35)·스토퍼 홈(r 43)이 들어간다 |
| 두께 | 낮을수록 (8~10) | 높아지면 혼 연결 기둥이 길어지고 팔이 올라간다 |
- 작은 볼베어링(608ZZ·6800ZZ·627ZZ 등)은 안 된다 — 가운데로 배선이 못 지나가고 기울기 모멘트를 못 버틴다.
- 사면: **바깥지름·안지름·두께·두 링의 볼트 원 지름과 구멍 수**를 재서 `brg_*`에 넣는다.
- 손으로 윗판을 흔들어 기울기 유격을 본다. 0.5°면 팔 끝 30cm에서 ~2.6mm — 크면 얇은 깊은홈 베어링(6818ZZ, 90×115×13)을 프린트 받침에 끼우는 쪽으로.
...
## 출력 전 실측 (scad의 `[실측]` 줄)
베어링(구매 후) · 벅부스트 부품 높이 · 젯슨 캐리어 고정 구멍 위치(지금 86×58 추정) ·
