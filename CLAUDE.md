...
- **ROS 없이 PC에서 도는 4대 자체검증 (총 768종)** (이 저장소의 규칙: 숫자는 젯슨에 올리기 전에 확인한다)
  `python tools/handeye_check.py` (보정 수학 45종) ·
  `python tools/eye_check.py` (보정→통합 배선 102종 — 카메라 둘 포함) ·
  `python ros2/tools/ros_selfcheck.py` (레거시 경계·URDF↔기구학 일치·보드계약 5층 데드맨·계약테스트/0속도정지·서보캘리 틱스팬↔NormLimits 물리범위·limits_deg 과대개방 감사·부품 전장하한/mount.x기하·깊이 거절·스탠드오프·프리셋/캘리·SimBase 물리한계·응답파서/프로토콜계약 등 528종) ·
...
- ⚠ **잡기의 교훈은 [`docs/개발일지-2026-09-11.md`](docs/개발일지-2026-09-11.md)** —
  같은 병의 잡기 판(48회 2성공 → 3회 3성공). **틀린 전제 셋**: ① 겨눠 온 화소에 집게가
  있다(롤을 만지면 무효 — 겨냥이 정확할수록 틀린 자리로 정확히 간다) ② 가장 가까운 것이
  표적이다(자기 손가락이다) ③ 거리를 맞힐 수 있다(가는 표적의 깊이는 정답 구간보다 넓게
  흔들린다 — **맞히지 말고 더듬어라**: 60%만 가서 물어보고 아니면 8mm 더).
