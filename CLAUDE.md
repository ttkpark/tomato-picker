...
- **ROS 없이 PC에서 도는 4대 자체검증 (총 675종)** (이 저장소의 규칙: 숫자는 젯슨에 올리기 전에 확인한다)
  `python tools/handeye_check.py` (보정 수학 45종) ·
  `python tools/eye_check.py` (보정→통합 배선 102종 — 카메라 둘 포함) ·
  `python ros2/tools/ros_selfcheck.py` (레거시 경계·URDF↔기구학 일치·보드계약 5층 데드맨·깊이 거절·스탠드오프·프리셋/캘리·토픽일치 등 435종) ·
