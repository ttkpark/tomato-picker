...
def main() -> int:
    ...
    def near_point(prev, R=110):
        ...
        if bgr is not None and bgr.shape[:2] == dep.shape[:2]:
            ...
            # ⚠ **초록 지지대도 뺀다**(2026-09-14): 면봉 바로 옆에 선 초록 막대를 표적으로
            #   골라 그걸 물고 "성공"(집게 4.7)이 찍혔다. 면봉은 흰색이고 지지대는 초록이다.
            hand = (((hsv[:, :, 0] > 115) & (hsv[:, :, 0] < 165) & (hsv[:, :, 1] > 55))
                    | (hsv[:, :, 2] < 55)).astype(np.uint8)
            # ⚠ 지지대(초록)는 **번지게 지우지 않는다** — 면봉이 그 안에 꽂혀 있어서 9×9로
            #   팽창시키면 면봉 아랫부분까지 깎여 덩이가 270화소로 쪼그라들고 네 걸음 만에
            #   놓쳤다(2026-09-14 06:52). 자기 손만 넓게 지우고 초록은 제 화소만 지운다.
            green = ((hsv[:, :, 0] > 35) & (hsv[:, :, 0] < 90) & (hsv[:, :, 1] > 60)).astype(np.uint8)
            own = cv2.dilate(hand, np.ones((9, 9), np.uint8)) | green
