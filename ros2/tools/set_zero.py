...
⚠ 영점을 다시 잡으면 저장된 프리셋의 **좌표 해석**이 바뀐다(관절값 자체는 그대로라
   재생 동작은 같다). 그리고 손-눈 보정(`~/arm_eye.json`) 및 파지 화면좌표(`~/grip_uv.json`)는
   **무효가 된다** — 도구 좌표계의 뜻과 롤/관절 영점이 바뀌기 때문이다. 그래서 잡기 전에
   파일을 백업한다.
"""
...
def main() -> int:
    ...
    if ok:
        print("⚠ 주의: 기구학 영점이 바뀌었으므로 ~/arm_eye.json(손-눈 보정) 및 "
              "~/grip_uv.json(파지 화면좌표)이 무효화됩니다. 필요 시 재측정하십시오.")
