...
class DepthView:
    ...
    def status(self) -> dict:
        ...
        out = {
            ...
            # 화면 밝기 (2026-09-18 발행기가 싣기 시작). 옛 발행기가 돌고 있으면
            # 없을 수 있으므로 None을 그대로 통과시킨다.
        ...
        # ⚠ 2026-09-17에 이 한 줄이 없어서 하루를 썼다. 점 검출(`target_check.find_dots`)은
        #   고리와 알맹이의 밝기 차 **30**을 요구하는데, 화면 최댓값이 그보다 낮으면
        #   마커가 눈앞에 있어도 0개가 나온다. 그때 "마커판이 시야 밖"이라고 읽었고
        #   그 오독이 손-눈 작업 전체를 멈춰 세웠다. 그래서 검출을 믿기 전에
        #   **밝기부터** 말하게 한다 — 이 저장소의 "게인 말고 크기부터 재라"와 같다.
        ...
        if p99 is not None and p99 < 30:
            out["color_warn"] = (
                f"화면이 어둡다(평균 {out.get('color_mean')}, 99백분위 {p99}). "
                "점 검출은 밝기 차 30을 요구하므로 이 화면에서는 **원리상** 0개가 "
                "나온다 — 0개를 '표적이 없다'로 읽지 마라. 조명을 켜거나 발행기의 "
                "D405_EXPOSURE_US·D405_GAIN을 올려라.")
        return out
