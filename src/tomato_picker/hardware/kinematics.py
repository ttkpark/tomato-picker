...
class ArmGeometry:
    ...
    # 2026-08-31 **실측**(자로 잼). 그 전까지는 도면 근사치였고, 그 오차가
    # 손-눈 보정을 통째로 못 풀게 만들고 있었다 — 특히 l3가 95 → 168로
    # 1.77배 틀렸다(카메라로 잰 불일치 1.8배와 정확히 일치했다).
    z0: float = 119.5   # 옛 근사치 55.0
    d0: float = -31.5   # 실측. **음수 = pan축보다 뒤쪽**(팔이 뻗는 +x의 반대)
    l1: float = 116.5   # 옛 116.0
    l2: float = 138.0   # 옛 135.0
    l3: float = 168.0   # 옛 95.0 ← 가장 크게 틀렸던 값
