...
class JetsonBase(MobileBase):
    ...
    def hold(self, vx: int = 0, vy: int = 0, w: int = 0) -> None:
        """지금부터 이 속도로 계속 가라(논블로킹). (0,0,0)이면 목표속도 0.

        브라우저가 키를 누르고 있는 동안, 또는 펄스 주행이 20ms마다 부른다.
        갱신이 끊기면 MotorLink가 0.5초 뒤 스스로 세우고, 그마저 못 가면
        펌웨어 데드맨이 세운다.

        ⚠ 0을 받아도 **하드 스톱(S)을 보내지 않는다.** 예전엔 보냈는데, 펄스 주행이
        OFF 구간마다 이걸 부르면서 초당 50번의 S가 나갔다. 펌웨어는 S마다
        ① 슬루를 무시한 즉시 정지 ② "ok S" 응답 ③ I2C 8채널 재기록을 하므로
        루프가 짓눌려 펄스가 불규칙해졌다(키보드 조작은 뗄 때 1번만 보내서
        멀쩡했다 — "톡톡과 키보드가 왜 다르냐"의 정체, 2026-08-10).
        목표만 0으로 두면 펌웨어 슬루가 부드럽게 세운다. 진짜 비상정지는 stop().
        """
        self._ensure_tuning()
        self._link.set_velocity(vx, vy, w)
