...
# 데드맨(400ms) 안에 재전송해야 한다. 0.15초는 여유가 2.7배뿐이라,
# Whisper STT(CPU 전코어)·YOLO가 도는 젯슨에서 파이썬 스레드가 한 번만
# 밀려도 400ms를 넘겨 펌웨어가 자동정지 → 다음 지령에 재출발한다. 실제로
# 3초 주행 중 "주웅 주웅 주우웅"으로 2번쯤 끊기는 게 실측됐다(2026-08-05).
# 0.05초로 낮춰 8배 여유를 준다 — 115200baud에 12바이트라 부하는 무시 가능.
BASE_DRIVE_RESEND_INTERVAL_SEC = 0.05
# 소프트 스타트: 정지→목표속도로 한 번에 때리면 인러시 전류가 순간 최대라
# 12V 레일이 무너진다. 이 시간 동안 START_FRACTION→1.0으로 선형 램프해서
# 같은 최고속도를 쓰면서도 기동 피크만 낮춘다.
BASE_DRIVE_RAMP_SEC = 0.5
BASE_DRIVE_RAMP_START_FRACTION = 0.35  # 너무 낮으면 정지마찰을 못 이겨 안 구른다
