...
LINE_PULSE_ON = 0.12      # 초: 한 펄스가 구동되는 시간
LINE_PULSE_PERIOD = 0.32  # 초: 펄스 반복 주기(= ON + 정지시간). ON과 같으면 연속
# 라인 유지 보정 게인. dy는 -1..1 정규화값, yaw는 도(deg) 단위.
LINE_DY_GAIN = 120.0
# ⚠ 회전 보정은 **기본 꺼짐**(0). 2026-08-09 실기에서 yaw가 발산했다.
# 메카넘은 비스듬히 서 있어도 게걸음 보정만으로 테이프를 따라 **게처럼** 갈 수 있어서,
# 회전 보정 없이도 주행 자체는 성립한다(자세는 손으로 맞춰 두면 그대로 유지됨).
# 각 추정을 안정화한 뒤 대시보드 [회전 보정 켜기/끄기]로 켜서 확인할 것.
LINE_YAW_GAIN = 0.0
